...
Native Linux `4.2.0-linux.4` is software-accepted on Ubuntu 24.04.5. The GTK4/Libadwaita desktop UI is single-instance and has explicit minimize/maximize/close controls. WARP safe-trial, token-scoped rollback, ownership protection, Tor Direct, Tor obfs4, isolated Firefox proxy profile, integrity-checked install, and the software Console Gateway path all passed runtime acceptance. The console path still has one field gate: DHCP/UDP/game/country behavior must be validated with a physical console connected.
