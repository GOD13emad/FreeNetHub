...
$checks.StatusExists=Test-Path -LiteralPath $status; if($checks.StatusExists){$st=Get-Content -LiteralPath $status -Raw -Encoding UTF8|ConvertFrom-Json;$checks.StatusShellVersion=$st.shellVersion;$checks.StatusShellVersionExpected=($st.shellVersion -eq '4.1.1')}
...
if($checks.ExeExists){
  ...
  $checks.FileVersion=$v.FileVersion; $checks.FileVersionExpected=($v.FileVersion -eq '4.1.1.0')
